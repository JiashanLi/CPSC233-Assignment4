...
// when move the car to a new location, set the old location to  null //
    protected void setnull( int location)
    {
      aTrack[location] = null;
    }

  
// check if the fuel of the car is empty // 
    public boolean checkfuel()
    {
      int i;
      for(i = 0; i < SIZE; i++)
          if(aTrack[i] != null)
            if(aTrack[i].isEmpty() == true)
              empty = true;
            else
              empty = false;
            return(empty);
